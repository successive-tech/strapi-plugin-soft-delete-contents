...
import { Layouts as Layout, Page } from '@strapi/admin/strapi-admin';
...
const RestorationBehavior: React.FunctionComponent = () => {
  ...
  const [initialSettings, setInitialSettings] = useState<any | undefined>(undefined);
  ...
  useEffect(() => {
    setIsLoading(true);
    get(`/${PLUGIN_ID}/settings`).catch((error: Error) => {
      setAlert({
        variant: 'danger',
